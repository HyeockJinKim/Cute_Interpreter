...
def run_func(op_code_node):
    ...
    def lamda(node):
        ...
        if node.next is None:
            return node
        variable = l_node.value # left
        varvalue = node.next #다음 괄호
        while variable is not None:
            KeywordTable.temptable[variable.value] = run_expr(varvalue) #다음 식들 run_Expr
            variable = variable.next # 다음값들로 진행
            varvalue = varvalue.next # 괄호의다음 부분
